...
const folderToReadFrom = '../data_scryfall/standard_scryfall';
const fs = require('fs');
...
function composed(strippedSets) {
	...
	returnString += '];\n\n'
	returnString += 'module.exports = combinedData;';
	// {\n';
	//
	// for(let elem of strippedSets) {
	// 	returnString += `  ${elem.setFileName},\n`
...
async function getFiles(folderToRead) {
	...
	const filename = 'combinedDataStandard';
